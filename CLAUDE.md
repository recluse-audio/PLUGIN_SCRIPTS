...
Build scripts call `regenSource.py` before building. It walks `SOURCE/`, `TESTS/`, and any `SUBMODULES/*/SOURCE` and `SUBMODULES/*/TESTS`, then writes `CMAKE/SOURCES.cmake` and `CMAKE/TESTS.cmake` with all discovered `.h/.cpp`. Users never hand-edit those CMake file lists — just add files and rebuild.
