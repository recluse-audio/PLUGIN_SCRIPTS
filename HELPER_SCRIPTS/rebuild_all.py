...
Usage examples:
  python build.py
  python build.py --target <PluginName> --config Release
...
def main() -> int:
    ...
    # If generator wasn't specified, we can't know for sure; choose a safe default on Windows.
    gen = args.generator
    multi = is_multi_config_generator(gen)

    if sys.platform.startswith("win"):
        # If user didn't specify generator, assume multi-config is likely with VS.
        if args.config is None:
            args.config = "Debug"
        build_cmd += ["--config", args.config]
    else:
        # Non-Windows: only pass --config if user explicitly provided it or generator is known multi-config (Xcode).
    ...
    if args.verbose:
        # Works for many generators; harmless if ignored.
        build_cmd += ["--verbose"]
